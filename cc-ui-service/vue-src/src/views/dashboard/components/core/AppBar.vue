      ...
      <template v-slot:activator="{ attrs, on }">
        ...
        >
          <v-badge
            ...
            overlap
            bordered
            v-if="num2str(notifications.length)"
          >
          ...
          <v-icon v-if="!num2str(notifications.length)">mdi-bell</v-icon>
        </v-btn>
      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toDashBoard()
      {
        if(localStorage.username == 'admin')this.$router.push("/dashboardAdmin");
