  ...
  <v-container>
      ...
      <v-divider></v-divider>
        <v-card class="mx-auto" max-width="100%" v-if="infoUpdated" style="background-color: rgb(232, 232, 232)">
            ...
            <v-card-actions>
                ...
                <v-btn text v-if="meetingInfo.status=='ReviewConfirmed'" v-on:click="publishFinalResult()">
    ...
    <v-dialog v-model="dialog" max-width="750" persistent>
        <v-card>
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="green darken-1" text v-on:click="review('TopicRelevant')" v-prevent-re-click>TopicRelevant</v-btn>
                <v-btn color="green darken-1" text v-on:click="review('LoadBalancing')" v-prevent-re-click>LoadBalancing</v-btn>
...
import preventReClick from '../../plugins/plugin'
export default {
